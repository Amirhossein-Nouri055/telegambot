...
# Bot token
TOKEN = "7695882385:AAH8uuK4V6FCTI6zZGzU76Ct_fn_cv6pQbc"
...
class ProductData(Base):
    ...
    ajrat = Column(Float, nullable=False)
    ajrat_type = Column(String, nullable=False)
    profit = Column(Float, nullable=False)
    profit_type = Column(String, nullable=False)
    accessories = Column(Float, nullable=False)
...
# Product data storage (for backward compatibility)
...
def save_product_to_db(message_id: str, product_info: Dict[str, Any]):
    """Save product data to database"""
    try:
        db = get_db_session()
        product = ProductData(
            message_id=message_id,
            weight=product_info['w'],
            ajrat=product_info['a'],
            ajrat_type=product_info['at'],
            profit=product_info['p'],
            profit_type=product_info['pt'],
            accessories=product_info['accessories'],
            timestamp=datetime.fromisoformat(product_info['timestamp']),
            channel_id=product_info['channel_id']
        )
        db.add(product)
        db.commit()
        db.close()
        logger.info(f"Product data saved to database for message {message_id}")
    except Exception as e:
        logger.error(f"Error saving product to database: {e}")

def get_product_from_db(message_id: str) -> Optional[Dict[str, Any]]:
    """Get product data from database"""
    try:
        db = get_db_session()
        product = db.query(ProductData).filter(ProductData.message_id == message_id).first()
        db.close()
        ...
        if product:
            return {
                'w': product.weight,
                'a': product.ajrat,
                'at': product.ajrat_type,
                'p': product.profit,
                'pt': product.profit_type,
        ...
        logger.error(f"Error getting product from database: {e}")
        return None

def get_all_products_from_db() -> Dict[str, Dict[str, Any]]:
    """Get all products from database"""
    try:
        db = get_db_session()
        products = db.query(ProductData).all()
        db.close()
        
        result = {}
        for product in products:
            result[product.message_id] = {
                'w': product.weight,
                'a': product.ajrat,
                'at': product.ajrat_type,
                'p': product.profit,
        ...
        logger.error(f"Error getting all products from database: {e}")
        return {}

def clean_old_products_from_db():
    """Remove old products from database"""
    try:
        threshold = datetime.now() - timedelta(days=90)
        db = get_db_session()
        old_products = db.query(ProductData).filter(ProductData.timestamp < threshold).all()
        removed_count = len(old_products)
        ...
        if removed_count > 0:
            logger.info(f"Removed {removed_count} old records from database")
    except Exception as e:
        logger.error(f"Error cleaning old products from database: {e}")

# Load product data (migrate from JSON to database)
def load_product_data():
    global PRODUCT_DATA_STORE
    try:
        # Try to load from database first
        PRODUCT_DATA_STORE = get_all_products_from_db()
        logger.info(f"Loaded {len(PRODUCT_DATA_STORE)} product records from database")
        
        # If database is empty, try to migrate from JSON file
        if not PRODUCT_DATA_STORE:
            try:
                with open('product_data.json', 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    if isinstance(data, dict):
                        for message_id, product_info in data.items():
                            save_product_to_db(message_id, product_info)
                        PRODUCT_DATA_STORE = data
                        logger.info(f"Migrated {len(data)} records from JSON to database")
...
# Save product data (now saves to database)
def save_product_data():
    # This function is kept for compatibility but now data is saved directly to database
    logger.info("Product data is now saved directly to database")

# Clean old data
def clean_old_data():
    # Clean from database
    clean_old_products_from_db()
    ...
    try:
        ...
        for channel_id in CHANNEL_IDS:
            try:
                ...
                PRODUCT_DATA_STORE[message_id] = product_info
                logger.info(f"Stored product data for message {message_id}: {product_info}")
...
async def button_callback(update: telegram.Update, context: telegram.ext.ContextTypes.DEFAULT_TYPE):
    ...
    # Try to get from database first, then from local store
    product_data = get_product_from_db(message_id)
    if not product_data:
...
async def db_status(update: telegram.Update, context: telegram.ext.ContextTypes.DEFAULT_TYPE):
    if str(update.effective_chat.id) != ADMIN_CHAT_ID:
        await update.message.reply_text("🔒 این دستور فقط برای ادمین قابل استفاده است!")
        return
    ...
    try:
        ...
        db = get_db_session()
        total_records = db.query(ProductData).count()
        
        threshold = datetime.now() - timedelta(days=7)
        recent_records = db.query(ProductData).filter(ProductData.timestamp > threshold).count()
        ...
        status_message = (
            ...
            f"• رکوردهای هفته اخیر: {recent_records}\n"
            f"• آخرین به‌روزرسانی: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            f"• نوع ذخیره‌سازی: PostgreSQL Database"
        )
        
        await update.message.reply_text(status_message)
    except Exception as e:
        await update.message.reply_text(f"❌ خطا در دریافت وضعیت دیتابیس: {e}")
...
def auto_backup():
    """Create automatic backup of product data"""
    try:
        import shutil
        from datetime import datetime
        backup_filename = f"product_data_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        if os.path.exists('product_data.json'):
            shutil.copy2('product_data.json', backup_filename)
