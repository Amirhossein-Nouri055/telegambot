...
GITHUB_REPO = os.getenv('GITHUB_REPO', 'your-username/your-repo')  # Replace with your actual repo
GITHUB_FILE_PATH = 'product_data.json'
...
async def github_get_file():
    """Get product data from GitHub repository"""
    if not GITHUB_TOKEN:
        logger.warning("GitHub token not configured, using local fallback")
        return None
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.info("GitHub file not found, starting with empty data")
                    return {}
                else:
...
async def github_save_file(data):
    """Save product data to GitHub repository"""
    if not GITHUB_TOKEN:
        logger.warning("GitHub token not configured, using local fallback")
        return False
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Error getting current file: {response.status}")
                    return False
            ...
            payload = {
                "message": f"Update product data - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                "content": content_b64
            }
            ...
            async with session.put(url, headers=headers, json=payload) as response:
                if response.status in [200, 201]:
                    logger.info("Successfully saved data to GitHub")
                    return True
                else:
                    error_text = await response.text()
                    logger.error(f"Error saving to GitHub: {response.status} - {error_text}")
                    return False
...
async def load_product_data():
    ...
    # Try GitHub first
    github_data = await github_get_file()
    if github_data is not None:
        PRODUCT_DATA_STORE = github_data if isinstance(github_data, dict) else {}
        logger.info(f"Loaded {len(PRODUCT_DATA_STORE)} product records from GitHub")
        return
    ...
    try:
        with open('product_data.json', 'r', encoding='utf-8') as f:
            ...
            logger.info(f"Loaded {len(PRODUCT_DATA_STORE)} product records from local file")
    except FileNotFoundError:
        logger.info("Product data file not found, creating new one")
        PRODUCT_DATA_STORE = {}
        await save_product_data()  # Create empty file
    except Exception as e:
        logger.error(f"Error loading product data: {e}")
        PRODUCT_DATA_STORE = {}
        # Try to create a backup and start fresh
        try:
            with open('product_data.json', 'w', encoding='utf-8') as f:
                json.dump({}, f, ensure_ascii=False, indent=2)
        except Exception as backup_error:
            logger.error(f"Error creating backup file: {backup_error}")

# Save product data
async def save_product_data():
    # Try GitHub first
    github_success = await github_save_file(PRODUCT_DATA_STORE)
    if github_success:
        logger.info(f"Product data saved successfully to GitHub ({len(PRODUCT_DATA_STORE)} records)")
        return
    ...
    try:
        ...
        if os.path.exists('product_data.json'):
            shutil.copy2('product_data.json', 'product_data_backup.json')
        
        with open('product_data.json', 'w', encoding='utf-8') as f:
            json.dump(PRODUCT_DATA_STORE, f, ensure_ascii=False, indent=2)
        logger.info(f"Product data saved successfully to local file ({len(PRODUCT_DATA_STORE)} records)")
    except Exception as e:
        logger.error(f"Error saving product data: {e}")
        # Try to restore from backup if available
        try:
            if os.path.exists('product_data_backup.json'):
                shutil.copy2('product_data_backup.json', 'product_data.json')
...
async def db_status(update: telegram.Update, context: telegram.ext.ContextTypes.DEFAULT_TYPE):
    ...
    total_records = len(PRODUCT_DATA_STORE)
    recent_records = 0
    threshold = datetime.now() - timedelta(days=7)
    
    for record in PRODUCT_DATA_STORE.values():
        try:
